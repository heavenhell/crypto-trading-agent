...
- ★ 止盈 ≠ 立刻平仓！「止盈X%」意思是等价格涨到目标再触发，不是现在平！
  你要先查当前价格，算出止盈触发价，然后用 close + price。
  例："止盈100%" → 当前价$590，触发价=$590×2=$1180 →
  propose_order(market='futures', action='close', symbol='BNB',
                side='long', price=1180)
  例："止盈50%，平一半" → 当前价$600，触发价=$600×1.5=$900 →
  propose_order(market='futures', action='close', symbol='ETH',
                side='long', amount=0.08, price=900)
- 平仓 close 参数说明：
